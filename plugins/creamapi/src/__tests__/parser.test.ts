...
describe("CreamAPI save parser", () => {
  it("parses achievements from INI fixture", async () => {
    const result = await parseCreamApiSave(join(fixturesDir, "achievements.ini"));
  ...
  it("returns error for nonexistent file", async () => {
    const result = await parseCreamApiSave("/nonexistent/achievements.ini");
