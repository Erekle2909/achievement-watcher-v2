...
describe("CODEX save parser", () => {
  it("parses achievements from INI fixture", async () => {
    const result = await parseCodexSave(join(fixturesDir, "achievements.ini"));
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    // SteamAchievements section is skipped, so we expect 2 achievements
  ...
  it("returns error for nonexistent file", async () => {
    const result = await parseCodexSave("/nonexistent/achievements.ini");
