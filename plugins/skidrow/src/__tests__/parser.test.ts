...
describe("SKIDROW save parser", () => {
  it("parses achievements from INI fixture", async () => {
    const result = await parseSkidrowSave(join(fixturesDir, "achievements.ini"));
  ...
  it("returns error for nonexistent file", async () => {
    const result = await parseSkidrowSave("/nonexistent/achievements.ini");
