...
interface JsonSaveEntry {
  earned: boolean;
  earned_time: number;
}

type JsonSaveFile = Record<string, JsonSaveEntry>;

/**
 * Parses Goldberg/EMPRESS-style JSON achievement save files.
 * Format: { "ACH_NAME": { "earned": true, "earned_time": 1700000000 }, ... }
 */
export async function parseJsonAchievementSave(
  filePath: string,
): Promise<ParseResult<Achievement[]>> {
  ...
  let data: JsonSaveFile;
  try {
    data = JSON.parse(raw) as JsonSaveFile;
  } catch {
